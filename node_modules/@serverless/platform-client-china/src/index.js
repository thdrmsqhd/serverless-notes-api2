...
class ServerlessSDK {
  ...
  constructLoginRootUrl(componentName, componentVersion) {
    let url = `${this.getDomain('dashboard')}/components/c`;
  ...
  async login(config = {}) {
    ...
    const loginRootUrl = this.constructLoginRootUrl(config.componentName, config.componentVersion);

    const auth0Queries = querystring.stringify({
      audience: `https://${config.auth0Domain}/userinfo`,
      response_type: 'code',
      client_id: config.auth0ClientId,
      redirect_uri: `${loginRootUrl}?transactionId=${transactionId}`,
      scope: scope.join(' '),
    });
    const loginUrl = `${loginRootUrl}?${auth0Queries}`;
