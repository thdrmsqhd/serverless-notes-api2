...
class ServerlessSDK {
  ...
  async saveUserMeta(userMeta) {
    return await resources.saveUserMeta(this, userMeta)
  }
  ...
  async publishSync(event) {
    return this.eventer.publish(event)
  }

  /**
   * Retrieve a Serverless Platform Event synchronously via HTTP API.
   * @param {string} uid UID of event to be fetched
   * @returns {object} An event object if a valid id was provided. All events share a common structure, detailed below. The only property that will differ is the `data` payload.
   * ```json
   * {
   ...
   * @returns {object} A dictionary with a `data` property that contains an array of up to `limit` events, starting after event `starting_after`. Each entry in the array is a separate `event` object. If no more events are available, the resulting array will be empty.
   * ```json
   * {
   *   "object": "list",
   *   "data": [
